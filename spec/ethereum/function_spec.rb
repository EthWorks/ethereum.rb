...
describe Ethereum::Function do
  ...
  context "calculates signature with tuple[2]" do
    let(:inputs_json1) { {"type" => "tuple[2]", "name" => "nomatter",
                          "components" => [ {
                                             "name": "var1",
                                             "type": "address"
                                            },
    ...
    let(:inputs) { [Ethereum::FunctionInput.new(inputs_json1)] }
    it "simple" do
      expect(Ethereum::Function.calc_signature("sam", inputs)).to eq "sam((address,int256)[2])"
    end
  end
